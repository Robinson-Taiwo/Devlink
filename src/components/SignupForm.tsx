...
const SignupForm = () => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>): void => {
        ...
        if (email === "") {
            ...
            createUserWithEmailAndPassword(auth, email, password).then(response => {
                ...
                setAlertText("Hi there!! Your signup is successful. You will be redirected to the login page shortly");
                setLoading(false)
                setTimeout(() => {
                    router.push("/login")
                }, 3000)
    ...
    return (
        <div className="flex flex-col w-[25.9rem]  tablet:w-[33rem] sphone:w-[100%] ">
            ...
            <form onSubmit={handleSubmit} noValidate className="flex sphone:w-[100%]   gap-[2rem]  flex-col "  >
                <div className="flex flex-col sphone:w-[100%] phone:w-[100%] gap-[0.33rem] ">
                    ...
                    <div className={`flex group focus-within:border-purple  focus-within:ring-purple focus-within:ring-3 focus-within:shadow-purple focus-within:shadow-sm focus-within:ring-opacity-[25%] gap-[1rem] items-center flex-center flex-row h-[4rem] px-[1.33rem]  ${emailError || passwordError || confirmError ? "border-[red]" : "border-border"} border-[0.083rem] py-[1rem] phone:w-[100%] sphone:w-[100%] rounded-[0.67rem]  `} >
                        ...
                        <input className=" placeholder:text-darkGrey phone:w-[20.92rem] tablet:w-[28rem] laptop:text-normal h-[2rem] sphone:w-[100%]" type="email" name="email" placeholder="e.g. alex@email.com" onChange={handleChangeEmail} value={email} />
                        {emailError && <p className="text-[red]  laptop:w-[13rem] text-center text-base mt-1">{emailError}</p>}
                    </div>
                ...
                <div className="flex flex-col   phone:w-[100%]  gap-[0.33rem]  " >
                    ...
                    <div className={`flex group focus-within:border-purple  focus-within:ring-purple focus-within:ring-3 focus-within:shadow-purple focus-within:shadow-sm focus-within:ring-opacity-[25%] gap-[1rem] items-center flex-center flex-row h-[4rem] px-[1.33rem]  ${emailError || passwordError || confirmError ? "border-[red]" : "border-border"} border-[0.083rem] py-[1rem] phone:w-[100%] sphone:w-[100%] rounded-[0.67rem]  `}>
                        ...
                        <input className=" placeholder:text-darkGrey phone:w-[20.92rem] tablet:w-[28rem] laptop:text-normal h-[2rem] sphone:w-[100%]" type="password" name="password" placeholder="At least .8 characters" onChange={handlePasswordChange} value={password} />
                        {passwordError && <p className="text-[red] laptop:w-[13rem] text-base text-center leading-[1.5rem] mt-1">{passwordError}</p>}
